...
;(function() {
	 ...
	 * environtment.
	 */
	emoji.prototype.replace_emoticons = function(str, options){
		var self = this;
		var colonized = self.replace_emoticons_with_colons(str);
		return self.replace_colons(colonized, options);
	};
	 ...
	 */
	emoji.prototype.replace_colons = function(str, options){
		var self = this;
		self.init_colons();
		options = options || {};
		...
		return str.replace(self.rx_colons, function(m){
			...
			if (idx.indexOf('::skin-tone-') > -1){
				...
				if (val){
					return self.replacement(val, idx, ':', options.jumbo, {
						'idx'		: skin_val,
						'actual'	: skin_idx,
						'wrapper'	: ':'
					});
				}else{
					return ':' + idx + ':' + self.replacement(skin_val, skin_idx, ':', options.jumbo);
				}
			}else{
				var val = self.map.colons[idx];
				return val ? self.replacement(val, idx, ':', options.jumbo) : m;
			}
		});
	};
	 ...
	 */
	emoji.prototype.replace_unified = function(str, options){
		var self = this;
		self.init_unified();
		options = options || {};
		...
		return str.replace(self.rx_unified, function(m, p1, p2){
			...
			if (val){
				...
				if (p2 == '\uD83C\uDFFF') idx = '1f3ff';
				if (idx){
					return self.replacement(val, null, options.wrapper, options.jumbo, {
						idx	: idx,
						actual	: p2,
						wrapper	: ''
					});
					}
				return self.replacement(val, null, null, options.jumbo);
			}

			val = self.map.unified_vars[p1];
			if (val){
				return self.replacement(val[0], null, options.wrapper, options.jumbo, {
					'idx'		: val[1],
					'actual'	: '',
					'wrapper'	: '',
				});
	...
	/** @private */
	emoji.prototype.replacement = function(idx, actual, wrapper, jumbo, variation, is_extra){
		var self = this;
		...
		if (typeof variation === 'object'){
			extra = self.replacement(variation.idx, variation.actual, variation.wrapper, jumbo, undefined, true);
		...
		if (self.supports_css) {
			if (self.use_sheet && img.px != null && img.py != null){
				...
				return '<span class="emoji-outer emoji-sizer'+klass+'"><span class="emoji-inner" style="'+style+'"'+title+' data-codepoints="'+img.full_idx+'">'+text+'</span></span>'+extra;
			}else if (self.use_css_imgs){
				return '<span class="emoji emoji-'+idx+klass+'"'+title+' data-codepoints="'+img.full_idx+'">'+text+'</span>'+extra;
			}else{
				return '<span class="emoji emoji-sizer'+klass+'" style="background-image:url('+img.path+')"'+title+' data-codepoints="'+img.full_idx+'">'+text+'</span>'+extra;
			}
		}
		return '<img src="'+img.path+'" class="emoji'+klass+'" data-codepoints="'+img.full_idx+'" '+title+'/>'+extra;
	};
	...
	emoji.prototype.init_emoticons = function(){
		...
		var emoticons = a.join('|');

		self.rx_emoticons = new RegExp(('(^|\\s|\\>)('+emoticons+')(?=$|[\\s|\\?\\.,!|\\<])'), 'g');
		self.init_only_emoji({emoticons: emoticons});
	};
	...
	// helpful regex for detecting if the string only has emoji/emoticons/shortnames
	emoji.prototype.init_only_emoji = function(options){
			var self = this;
			options = options || {};
			self.inits.only_emoji = self.inits.only_emoji || {};
			if (self.inits.only_emoji.unicodes && self.inits.only_emoji.emoticons) return;
			self.inits.only_emoji.unicodes = self.inits.only_emoji.unicodes || options.unicodes;
			self.inits.only_emoji.emoticons = self.inits.only_emoji.emoticons || options.emoticons;
			if (!self.inits.only_emoji.unicodes || !self.inits.only_emoji.emoticons) return;

			var unicodes = self.inits.only_emoji.unicodes;
			var variantsUnicode = '\uD83C[\uDFFB-\uDFFF]';
			var emoticons = self.inits.only_emoji.emoticons;

			self.rx_only_emoji = new RegExp('^(\\s|'+unicodes+'|'+variantsUnicode+'|\:[a-zA-Z0-9-_+]+\:|'+emoticons+')+$', "g");
			self.inits.only_emoji.unicodes = 1;
			self.inits.only_emoji.emoticons = 1;
	}
	...
	emoji.prototype.init_unified = function(){
		...
		self.rx_unified = new RegExp('('+unicodes+')(\uD83C[\uDFFB-\uDFFF])?', "g");
		self.init_only_emoji({unicodes: unicodes});
	};
